...
def softmax_loss(Z, y):
    ...
    eps = 1e-10
    cross_entropy = -np.log(softmax_scores[np.arange(N), y] + eps)
...
def softmax_regression_epoch(X, y, theta, lr = 0.1, batch=100):
    ...
    for i in range(0, num_examples, batch):
        ...
        probs[range(batch), y_batch] -= 1 # Z - I_y
        grad = np.dot(X_batch.T, probs) / batch # 1/B * X^T * (Z - I_y)
...
def nn_epoch(X, y, W1, W2, lr = 0.1, batch=100):
    ...
    for i in range(0, num_examples, batch):
        ...
        Z1 = X_batch
        Z2 = np.maximum(0, np.dot(Z1, W1)) # relu
        Z3 = np.dot(Z2, W2)

        exp_Z3 = np.exp(Z3 - np.max(Z3, axis=1, keepdims=True))
        norm_exp_Z3 = exp_Z3 / np.sum(exp_Z3, axis=1, keepdims=True)
        G3 = norm_exp_Z3 # not real G3
        G3[range(batch), y_batch] -= 1 # S - I_y, real G3
        G2 = np.dot(G3, W2.T)

        W2_grad = np.dot(Z2.T, G3) / batch
        G2[Z2 <= 0] = 0 # G2 elementwise multiply 1{Z2 > 0}
        W1_grad = np.dot(Z1.T, G2) / batch
